...
node_t *addTree(node_t *t1, node_t *t2)
{
	// before traversing check if they are empty
	if (isEmpty(t1) && isEmpty(t2))
	{
		return NULL;
	}
	// if tree 1 isnt empty return the root there
	if (!isEmpty(t1) && isEmpty(t2))
		return t1;
	// if tree 2 isnt empty return the root there
	if (isEmpty(t1) && !isEmpty(t2))
		return t2;
	...
	// traverse the same spots in both trees recursively
...
int isEmpty(node_t *root)
{
	return (root == NULL); // return condition result
}
// TESTING PURPOSES
//  void preorderD(node_t *root)
//  {
//  	if (root != NULL)
//  	{
//  		printf("%d\t", root->data);
